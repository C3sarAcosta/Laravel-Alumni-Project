function charts_four(array_efficiency_work_activities, array_academic_training, array_usefulness_professional_residence,
    array_study_area, array_title, array_experience, array_job_competence, array_positioning, array_languages, array_recommendations, 
    array_personality, array_leadership, array_others) {

    var labels_efficiency_work_activities = array_efficiency_work_activities.map(a => a.efficiency_work_activities);
    var data_efficiency_work_activities = array_efficiency_work_activities.map(a => a.total);
    var colors_efficiency_work_activities = randomColor({luminosity: 'light',count: data_efficiency_work_activities.length});
    
    var labels_academic_training = array_academic_training.map(a => a.academic_training);
    var data_academic_training = array_academic_training.map(a => a.total);
    var colors_academic_training = randomColor({luminosity: 'light',count: data_academic_training.length});

    var labels_usefulness_professional_residence = array_usefulness_professional_residence.map(a => a.usefulness_professional_residence);
    var data_usefulness_professional_residence = array_usefulness_professional_residence.map(a => a.total);
    var colors_usefulness_professional_residence = randomColor({luminosity: 'light',count: data_usefulness_professional_residence.length});

    var labels_study_area = array_study_area.map(a => a.study_area);
    var data_study_area = array_study_area.map(a => a.total);
    var colors_study_area = randomColor({ luminosity: 'light', count: data_study_area.length });
   
    var labels_title = array_title.map(a => a.title);
    var data_title = array_title.map(a => a.total);
    var colors_title = randomColor({ luminosity: 'light', count: data_title.length });    

    var labels_experience = array_experience.map(a => a.experience);
    var data_experience = array_experience.map(a => a.total);
    var colors_experience = randomColor({ luminosity: 'light', count: data_experience.length });

    var labels_job_competence = array_job_competence.map(a => a.job_competence);
    var data_job_competence = array_job_competence.map(a => a.total);
    var colors_job_competence = randomColor({ luminosity: 'light', count: data_job_competence.length });

    var labels_positioning = array_positioning.map(a => a.positioning);
    var data_positioning = array_positioning.map(a => a.total);
    var colors_positioning = randomColor({ luminosity: 'light', count: data_positioning.length });

    var labels_languages = array_languages.map(a => a.languages);
    var data_languages = array_languages.map(a => a.total);
    var colors_languages = randomColor({ luminosity: 'light', count: data_languages.length });

    var labels_recommendations = array_recommendations.map(a => a.recommendations);
    var data_recommendations = array_recommendations.map(a => a.total);
    var colors_recommendations = randomColor({ luminosity: 'light', count: data_recommendations.length });

    var labels_personality = array_personality.map(a => a.personality);
    var data_personality = array_personality.map(a => a.total);
    var colors_personality = randomColor({ luminosity: 'light', count: data_personality.length });

    var labels_leadership = array_leadership.map(a => a.leadership);
    var data_leadership = array_leadership.map(a => a.total);
    var colors_leadership = randomColor({ luminosity: 'light', count: data_leadership.length });

    var labels_others = array_others.map(a => a.others);
    var data_others = array_others.map(a => a.total);
    var colors_others = randomColor({ luminosity: 'light', count: data_others.length });
    ...
    //Pie 11 -- 8
    var pieChartCanvas11 = $('#pieChart10').get(0).getContext('2d')
    var pieData11 = {
    ...
    //Pie 12 -- 9
    var pieChartCanvas12 = $('#pieChart11').get(0).getContext('2d')
    var pieData12 = {
    ...
    //Pie 13 -- 10
    var pieChartCanvas13 = $('#pieChart12').get(0).getContext('2d')
    var pieData13 = {
